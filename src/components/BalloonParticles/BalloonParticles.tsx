...
import { BoxGeometry, BufferGeometryUtils, Color, Group, InstancedMesh, Mesh, MeshStandardMaterial, Object3D } from "three";
...
const BalloonParticles = ({count = 10000, scale = 1, color = "0xffffff",rotation = [Math.PI / 2, Math.PI, Math.PI]} : BalloonParticlesType) => {
    ...
    useFrame(({clock}) => {
        ...
        balloonStructure?.forEach((balloonObject, index) => {
            ...
            balloonObject.position.set(Math.cos(angle), 0, Math.sin(angle)).multiplyScalar(balloonObject.userData.radius).setY(balloonObject.userData.height);
